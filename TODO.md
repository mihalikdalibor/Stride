...
- [ ] 🔥 **Štatistiky — prerobiť logiku streakov s udalosťami** (M) — dnes sa do streaku rátajú aj dni, kde boli len udalosti z kalendára (škola, práca), lebo skončená udalosť = hotová (`statsItems.ts` `eventToItem`). Nepôsobí to logicky — streak by mal odrážať vlastnú aktivitu, nie to, že bol rozvrh. Súvisí s „Jemnejšie nastavenie udalostí v štatistikách" vyššie a hotovým „Udalosti v štatistikách" (napr. udalosti rátať do hodín/kategórií, ale nie do streakov; alebo prepínač na feed). **Presná logika sa ešte vymyslí.**
