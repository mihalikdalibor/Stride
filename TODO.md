...
- [ ] **Jemnejšie nastavenie udalostí v štatistikách** (S–M) — dnes stačí globálny prepínač; neskôr prepínač priamo na feede (`calendar_feeds.count_in_stats`, migrácia) a prípadne na konkrétnej udalosti (vylúčiť zo štatistík bez zmazania z kalendára).
- [ ] **Úprava pripojeného kalendára** (S) — premenovanie a zmena URL (teraz sa dá len zmeniť kategória, obnoviť alebo odpojiť).
- [ ] 🔥 **Udalosti v mesačnom/ročnom kalendári — preveriť a prerobiť** (S–M) — číslo pod dňom v mesačnom kalendári dnes ráta **len úlohy** (`CalendarView.vue` `monthCells` → `dayTasks.length`); udalosti z kalendára sa v mriežke vôbec nezobrazia (načítajú sa až po otvorení dňa). Je to neprehľadné, lebo aj udalosť je aktivita. Nápad: každá udalosť = +1 do čísla (napr. 5 hodín v škole + pracovná zmena + posilka = 7). Treba doriešiť: či sa rátajú aj celodenné, či/ako ovplyvnia farebnú bodku stavu dňa, rovnako ročný pohľad, a načítanie udalostí za celý viditeľný rozsah (nie len pre otvorený deň). **Presná logika sa ešte dohodne.**
...
## Ručné udalosti / série — ďalšie kroky
- [ ] **Zmena pravidla série** (M) — dnes sa dá séria len zmazať („túto a nasledujúce") a vytvoriť znova; chýba úprava dní/dátumu konca na existujúcej sérii.
- [ ] **Späť pri mazaní série** (S) — „túto a nasledujúce" maže bez undo (jednotlivý výskyt undo má); doplniť hromadné vrátenie.
- [ ] **Mesačné opakovanie** (S) — nový formulár ponúka denne/týždenne; mesačne ostalo len v starom `repeat` (spawn po dokončení).
- [ ] **Späť pri konverzii aktivita↔udalosť** (S) — konverzia vloží nový typ a zmaže starý (pri chybe sa vráti), ale nemá tombstone „Späť"; stratí sa stav hotové/priorita.
- [ ] **Viac druhov streakov** (M) — napr. „aspoň raz za týždeň v škole/práci"; dnes je len jeden globálny streak + vylúčenie kategórie (`categories.exclude_from_streak`).
...
## Hotovo
- [x] **Streak bez fixných povinností** — udalosti (z kalendára aj ručné) sa do streaku nerátajú nikdy, kategória sa dá vylúčiť prepínačom „Počítať do streaku" v jej paneli (`categories.exclude_from_streak`, migrácia v README; `ti-flame-off` indikátor v riadku). Deň, kde ostanú len vylúčené veci, streak nepreruší ani nepredĺži. Výpočet vytiahnutý do `src/lib/streak.ts`. Percentá, grafy, hodiny a farby dní sa nemenia.
- [x] **Ručné udalosti + viac dní + série** — spoločný formulár `ItemForm.vue` (Domov aj detail dňa) s prepínačom **Aktivita / Udalosť** nad kategóriami; udalosť je fixná (bez checkboxu, bez priority, bez času = celodenná), ukladá sa do `calendar_events` s `feed_id = null` (vlastná kategória + poznámka, editovateľná ťuknutím, mazanie so Späť, sync ju nepremaže). Sekcia „Kedy": tento deň / viac dní (`MultiDatePicker.vue`) / opakovať (denne alebo týždenne vo vybrané dni, do dátumu, max 366 výskytov) — výskyty opakovania sa vytvoria dopredu so spoločným `series_id` a úprava/mazanie majú rozsah „len túto / túto a nasledujúce"; ručne vybrané dni ostávajú samostatné položky (aby sa zmeškaná dostala do „Po termíne"). Výskyty série sa nezobrazujú v sekcii „Po termíne". Menu `+` sa premenovalo na „Pridať".
- [x] **Udalosti v štatistikách** — skončená udalosť = hotová, budúca = naplánovaná; rátajú sa do metrík (done/total, %, streaky, graf, cieľ, heatmapa, kategórie počet + hodiny) spolu s úlohami rovnakej kategórie, celodenné sa preskakujú. Globálny prepínač „Rátať udalosti z kalendára" v Nastaveniach (`statsPrefs.ts`, localStorage). Udalosti sa pre Štatistiky načítavajú stránkovane za celý rozsah (`fetchStatsRange`). Farby dní v kalendári ostávajú len z úloh.
- [x] **Aktivity cez polnoc** — koniec skôr ako začiatok = končí nasledujúci deň (`duration_min` + 24 h); riadok aj formulár ukazujú `+1` (napr. `22:00–03:00 +1`).
