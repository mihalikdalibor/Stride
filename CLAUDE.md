...
- **Connected calendars (iCal)**: `calendar-sync` Edge Function downloads + parses a feed with the caller's JWT (browsers can't fetch feeds — no CORS), expands recurrences (−60/+365 days) and upserts into `calendar_events`. The `calendars` store reads events, writes feed settings + the per-event `hidden` flag, and does full CRUD on **manual** events (`feed_id = null`, added from the app); it re-syncs feeds older than 30 min once per session (`init()`). See README for deploy.
...
- `tasks`: `id, user_id, title, task_date (date), task_time (time, nullable), duration_min (int, nullable), priority (bool), repeat ('none'|'daily'|'weekly'|'monthly'), status ('todo'|'done'), category_id (nullable FK), note, position (int), series_id (uuid, nullable), created_at, completed_at`
- `categories`: `id, user_id, name, color, position (int), exclude_from_streak (bool), created_at`. `position` orders the chips/filter and the categories sheet (drag & drop reorder via `vuedraggable`); new DBs run the `position` migration in `README.md`. `exclude_from_streak` = its tasks don't count toward the streak (toggle in the category's panel in `CategoriesSheet.vue`, indicated by `ti-flame-off` on the row) — for fixed obligations like work, which say nothing about discipline. It affects **only** the streak, not completion %, charts, hours or day colors.
...
- `calendar_events`: `id, user_id, feed_id (nullable FK, on delete cascade), uid, title, location, note, starts_at, ends_at (timestamptz), all_day, hidden, category_id (nullable FK, on delete set null), series_id (uuid, nullable), synced_at`, unique `(feed_id, uid)`. One occurrence per row (`uid` = `UID|RECURRENCE-ID` for feed recurrences, a fresh uuid for manual ones). All-day events store the date at `00:00Z` with an exclusive end date. **`feed_id = null` = a manually added event** ("Udalosť" in the add form): fully editable (tap the row), really deleted (not hidden) with undo, carries its own `category_id` and `note`, and is included in the JSON backup (`src/lib/backup.ts`) since it can't be re-synced from a feed; `calendar-sync` only touches rows of its own feed, so manual events survive syncs. **Events are fixed, not tasks:** no checkbox, they're interleaved into `DayList` by start time (all-day first) but **don't count** toward done/total, the week chart on Home/Calendar, or the **streak**. They *do* color the Calendar day dots and can make an otherwise empty day show one (events are never "missed" — a past events-only day reads as done); the Calendar grid loads them for its whole span via `calendars.fetchGridRange` (paged, kept in `gridEvents` so the day sheet's `events` isn't replaced). **Stats** do count them (unless the global "Count events" toggle in Settings is off — `src/lib/statsPrefs.ts`, localStorage): `src/lib/statsItems.ts` maps tasks and events to a common `StatItem`; a timed event is done once it has ended, planned before, takes its own or its feed's category, all-day events are skipped. Stats loads events for its whole range via `calendars.fetchStatsRange` (paged past PostgREST's 1000-row cap, kept in `statsEvents` so Home's `events` isn't replaced). "Remove" sets `hidden = true` (swipe ← or tap → remove, with undo); sync never sends `hidden`, so it survives re-syncs.
- Notes:
  ...
  - `duration_min` stores the **end** time as minutes after the start (the edit/add form has "From"/"To" hour:minute selects; `DayList.vue` saves `duration_min = end − start` when both are set). **An end earlier than the start means the task ends the next day** (`durFrom` adds 24h, e.g. 22:00 → 03:00 = 300 min); equal start/end = no end (`null`). It is meaningless without `task_time`. The row renders the range, e.g. `14:00–15:30` (start + duration), or just `14:00` when no end; an end past midnight wraps and gets a `+1` suffix (`22:00–03:00 +1`), and the form shows the same `+1` next to "To". The task still belongs to its start day only (not split across days; stats/category hours count the whole duration on `task_date`). Nullable; same migration block in `README.md`.
  ...
  - `series_id` groups the occurrences of one **repeat rule**; days picked by hand stay independent rows (so a missed one still reaches the overdue section). The add form ("Kedy") creates either one day, several days picked in `MultiDatePicker.vue`, or a rule (`src/lib/recurrence.ts` `expandDates`: daily / weekly on picked weekdays, until a date, capped at 366) — the occurrences are **materialized up front** as ordinary rows sharing a `series_id`. Editing or deleting one asks for the scope in the form: "Len túto" or "Túto a nasledujúce" (`updateSeries`/`deleteSeries`, `updateEventSeries`/`deleteEventSeries`). Changing a rule isn't supported — delete "and following" and add it again. Occurrences of a series are **left out of the overdue section** (a missed repeating activity stays on its day). Row shows a 🔁 (`ti-repeat`) indicator.
  - `repeat` is the **legacy** recurrence (`none|daily|weekly|monthly`): when a task with `repeat≠none` is toggled **done**, the store (`toggleTask` → `spawnNextOccurrence`) inserts the next occurrence (`src/lib/dates.ts` `nextRepeatDate`) and **clears `repeat` on the completed one** so re-toggling never spawns a duplicate. Existing rows keep working, but the form no longer sets it — new repeats use `series_id`; editing such a task shows a "Opakuje sa po dokončení · Vypnúť" row that clears it. Default `'none'`; same migration block in `README.md`.
...
|---|---|---|
| green (`success`) | completed task | green checkbox / strikethrough item |
| red (`danger`) | **two uses:** (a) past day that had tasks but wasn't finished = "missed"; (b) the "today" marker | red filled circle / red number (today) — different channels so they don't clash |
| gray (`tertiary`) | planned (today/future, not yet judged) | gray dot / uncategorized slice |
| none | day had nothing on it | no dot |

To avoid the today/missed red clash: **today** = red filled circle around the number; **status** = a small dot *under* the number.

**Day dots are category-colored** (`src/lib/dayColors.ts`): the dot's **hue** is a pie of the categories of everything on that day — one equal slice per item, same-category items merged into one arc, uncategorized items a neutral gray slice, capped at 6 slices. Its **treatment** carries the status from `dayStatus` (`src/lib/status.ts`): `done` = solid, `planned` = 60% opacity, `missed` = an outline ring (a `radial-gradient` mask, `.is-missed` in `app.css`). The same colors fill the Home week bars (one segment per task, done solid at the bottom, the rest at 50% opacity).
...
- Title "Tento týždeň" + date range. Right: `‹ ›` (prev/next week) + `+` (menu: "Pridať" opens the add form on today, "Pripojiť kalendár" opens the iCal sheet). Swipe left/right also switches weeks.
- Week summary: "X / Y" done + thin progress bar.
- Mini 7-day bar chart (Po→Ne): bar height = total tasks that day, one segment per task in its category color (uncategorized = neutral gray) — done ones solid at the bottom, the rest at 50% opacity above, so the filled share still reads as "done". Events don't count toward the bars. Today's letter is red. Empty days show only a baseline dot.
- Agenda list Po→Ne (each day = header with name + date + "done/total", then task rows).
  ...
  - Add/edit form = `ItemForm.vue` (used by Home + Calendar day detail, for tasks and manual events): title, note, "Od"/"Do" time selects, an **Aktivita / Udalosť** segment (an event has no checkbox and no priority; without a time it's all-day), the category picker, and — when adding — the "Kedy" section (tento deň / viac dní / opakovať). Editing a series item adds the "Len túto / Túto a nasledujúce" scope segment; the type segment is disabled there.
...
- **Year**: 12 mini-month grids (3 columns), vertical infinite scroll across years. Each day with activity gets a small **category-colored** dot (see "Color logic": a pie of that day's categories, treatment by status). Today = red filled circle. Current month name = red. Tap a month → zoom to month view.
- **Month**: full grid, weekday header `po ut st št pi so ne` (Monday-start). Each day = number + category-colored dot + small task count (the count stays tasks-only). Today = red filled circle. Vertical infinite scroll between months. Tap a day → day detail.
...
- **Streak** lives in `src/lib/streak.ts` (not in the view): a day counts when everything judged on it is done, an unfinished past day breaks it, a day with nothing to judge is skipped. Events **never** count and neither do tasks from a category with `exclude_from_streak` — such a day stays "empty", so it neither extends nor breaks the streak.
